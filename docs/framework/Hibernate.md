...
1.  创建Hibernate的核心配置文件  hibernate.cfg.xml文件

2.  配置数据库连接信息 （数据库账号 密码 连接池…）
...
3.  创建与表相映射的类 提供get set方法

4.  创建映射文件  （名.hbm.xml）

5.  创建 HibernateUtils 获取连接对象 
...
6.  调用 session.get(类.class, 主键值);   2L

## 02 映射配置

+ class标签 ![](Hibernate.assets/9D2D2597-E4D1-45F8-91AD-87C1BBD1B315.png)
+  ID标签![img](Hibernate.assets/E4D06C66-4051-46EB-94A9-D9D636975C02.png)
+  property标签![img](Hibernate.assets/9F40963B-988D-4AB4-9D1D-F5EA9ECCAD9F.png)
...
*  查询方法  :    
  1. get（T.class,id）   查询之后，返回的是真实对象本身   没有查询到指定的id,返回的是一个空值
  2. load（T.class,id）  查询之后返回的是一个代理对象，使用的是第三方的代理机制，javassist.jar  没有查询到结果直接报一个异常（ObjectNotFoundException）
*   修改方法 :  void update(Object obj)   
   1. 直接创建对象修改  如果没有指定其它的字段，会把其它的字段设置为null  ***不建议使用*** 
   2.  查询之后再修改  修改了某一个字段，不会把其它的字段设置为null
*   删除方法: void delete(Object obj) 
   1.  直接创建对象删除  不支持级联删除
   2.  查询之后再删除  支持级联删除
*   保存或更新方法 : void saveOrUpdate(Object obj)  
   1.  没有设置id，是保存的操作
   2.  设置了id是修改的操作，如果设置的id没有，就会报错
...
>
> 添加：![0F996219-6943-4027-A217-1A1546569022](Hibernate.assets/0F996219-6943-4027-A217-1A1546569022.png)
>
>  修改：![684A9EAA-D34F-465E-A6BC-5C37E2CE1613](Hibernate.assets/684A9EAA-D34F-465E-A6BC-5C37E2CE1613.png)
>
>  删除：![394EC5C4-E697-4A8C-86BB-F02255BBC96F](Hibernate.assets/394EC5C4-E697-4A8C-86BB-F02255BBC96F.png)
 ...
 **多表查询：**不需要写外键相等的条件 （查询结果为 list集合 存放object数组）
...
> ```
>
> 迫切内连接（left join）: - 在普通内连接inner join 后添加一个关键字fetch  （通过hibernate将另一个对象的数据，封装该对象中）
>
> ```java
>  @Test
>     public void tset8(){
